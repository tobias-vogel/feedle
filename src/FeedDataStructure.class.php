...
//    $result .= '    <ul>' . "\n";
    ...
    foreach ($files as $file) {
      ...
      $result .= '      <li id="' . $feedId . '-' . $data['timestampid'] . '"><button onclick="archiveFeedItem(\'' . $feedId . '\', \'' . $data['timestampid'] . '\')"><img src="assets/recycle.png" alt="activity indicator"></button>' . (file_exists('cache/feeds/' . $feedId . '/favicon.ico') ? '<img width="16" src="cache/feeds/' . $feedId . '/favicon.ico" alt="favicon for this feed"/>' : '') . ' <a href="' . $data['uri'] . '">' . stripcslashes($data['title']) . '</a></li>' . "\n";
    }
